...
// >>> AJOUT : import de la route d'auto-sync DDB
import { installSyncDdbRoute } from "./routes/sync-ddb";
// <<< AJOUT
...
export default async function (
  ...
  if (process.env.NODE_ENV === "development") {
    (mustacheEngine as any).cache._max = 0;
    cacheMaxAge = 0;
  }
  ...
  // >>> AJOUT : montage de la route d'auto-sync DDB (après body-parser)
  installSyncDdbRoute(app);
  console.log("[sync-ddb] route mounted at /admin/sync-ddb");
  // <<< AJOUT
